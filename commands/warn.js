...
module.exports.run = async (bot, message, args) => {
  ...
  //!warn @daeshan <reason>
  if(!message.member.hasPermission("MANAGE_MESSAGES")) return message.reply("Não consegues fazer lhe isso!");
  let wUser = message.guild.member(message.mentions.users.first()) || message.guild.members.get(args[0])
  if(!wUser) return message.reply("Por favor mencione a pessoa.");
  if(wUser.hasPermission("ADMINISTRATOR")) return message.reply("Ele é muito bacano");
  ...
  if(warns[wUser.id].warns == 100){
    ...
    message.reply(`<@${wUser.id}> foi kickado.`)
  }
